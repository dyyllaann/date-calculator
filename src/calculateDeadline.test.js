...
test("returns unmodified date when options are false", () => {
  ...
  const nextBusinessDay = false;
  const result = calculateDeadline(date, 0, skipWeekends, skipHolidays, nextBusinessDay);
...
test("returns date + 1 day when options are false", () => {
  ...
  const skipHolidays = false;
  const nextBusinessDay = false;
  const result = calculateDeadline(date, 1, skipWeekends, skipHolidays, nextBusinessDay);
...
test("returns date + 4 day when skipWeekends is true", () => {
  ...
  const skipHolidays = false;
  const nextBusinessDay = false;
  const result = calculateDeadline(date, 2, skipWeekends, skipHolidays, nextBusinessDay);
